...
function draw_wall(i, j, w)
  if wall_at(i, j, w) then love.graphics.rectangle("fill", (i + M - 1) * CELL_W, (j + M - 1) * CELL_H, CELL_W, CELL_H) end
end

function draw_walls(w, h)
  love.graphics.setCanvas(WALLS_CANVAS)
  love.graphics.setColor(0, .1, .6)
  for j = 1, h do for i = 1, w do draw_wall(i, j, w) end end
  for _x = CELL_W, FRAME_W, CELL_W do love.graphics.line(_x, 0, _x, FRAME_H) end
  for _y = CELL_H, FRAME_H, CELL_H do love.graphics.line(0, _y, FRAME_W, _y) end
...
return {
  ...
  draw = function(self)
    ...
    for _, pit in pairs(self.state.pits) do pit:draw(CELL_W) end
    do
  ...
  love_update = function(self, dt)
    local ep = {}
    for _, eaten_pit in ipairs(self.state.eaten_pits) do if not eaten_pit:update(dt) then ep[#ep + 1] = eaten_pit end end
