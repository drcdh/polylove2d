...
function grid.process_input(keyboard, player_state, dt)
  ...
  if di ~= 0 or dj ~= 0 then return string.format("move:%s,%d,%d", player_state.name, di, dj) end
  return nil
end
...
function grid.draw(game_state, player_state)
  ...
  do
    ...
    for player_name, player in pairs(game_state.players) do
      ...
      love.graphics.print(
        string.format("%s: %d", player_name, game_state.player_scores[player_name]), 20, _y)
      _y = _y + 20
    end
  end
end
